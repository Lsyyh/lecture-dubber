...
- **实时同传模式**（2026-09-06 落地，双模式 WebUI）：首页 `/` 是模式跳转页，`/realtime` 实时同传、`/offline` 离线配音。实时链路：进程级 WASAPI loopback 捕获（**proc-tap**，无需管理员）→ 16k mono 重采样 → faster-whisper 分块流式 ASR（whisper_streaming 模式：每 tick 全窗重转写 + `StablePrefixBuffer` LCP 稳定提交 + 时间锚去重）→ 子句切分 → `RealtimeTranslator`（复用 translate.py 的 json_schema HTTP 层，同传 prompt + 模式提示）→ TTS → 声卡播放。纯逻辑在 `realtime_text.py`，编排（5 线程 + 有界队列 + 延迟控制器 + 事件 trace）在 `realtime_pipeline.py`，模型缓存模块级复用（二次开会话不重载）。
- **实时模式关键实测结论**：① WASAPI 进程 loopback 采样在**会话音量之后**——静音/音量0 会杀死捕获信号；故"降低原声"是把会话音量降到 `realtime_duck_volume`(0.08) + 捕获端数字增益 1/0.08 补偿，停止时恢复原音量。② **CosyVoice-300M-SFT 单机 RTF≈1**（fp16/fp32 都一样，短句 per-call 开销 2-3s），无实时余量；实时 TTS 默认 **Piper**（CPU，zh_CN-huayan，RTF≈0.05，`realtime_tts_backend: piper`），CosyVoice 为配置可选品质档。③ 实时 ASR 用 **faster-whisper-medium + int8_float16**（`realtime_asr_model`，ModelScope gpustack 下载）+ tick 1.3s，给 TTS 让 GPU。④ 优化后实测：12/12 子句全配音 0 丢弃，端到端延迟中位数 3.2s / p95 4.8s（SilentPlayer 文件回放 E2E，无声验证——**音频实验一律不得外放**）。
- **实时模式 API**：`GET /api/realtime/processes`（pycaw 枚举有声进程，注意 COM 必须在 CoInitialize 作用域内用完句柄）、`POST /api/realtime/start {pid, duck_original}`、`GET /api/realtime/status`、`POST /api/realtime/stop`。会话 trace 写 `outputs/realtime/<ts>/events.jsonl`。依赖装 `pip install -e '.[realtime]'`（proc-tap/SoundCard/pycaw/psutil/piper-tts）。
