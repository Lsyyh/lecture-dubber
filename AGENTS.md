...
- **LLM/VLM 服务**：`tools\llama-cpp` 的 llama-server（CUDA 构建）+ **Qwen3-VL-2B-Instruct Q8**（ModelScope，含 mmproj），一个服务同时承担翻译和字幕质控，OpenAI 兼容接口 127.0.0.1:8000/v1；启动参数带 `-np 4 -c 16384`（配合 `translate_workers: 4` 的并发翻译）和 `--chat-template-kwargs {"enable_thinking":false}`。多模态请求的 `response_format` 不生效，模型输出 JSON 可能带 ``` 围栏——解析一律走 `utils.extract_json`。
...
- **TTS 状态**（待继续）：CosyVoice1-300M-SFT（50Hz，实测 ~5.5s/句）仍是默认；CosyVoice2-0.5B 实测反而慢 4 倍（24s/句，无 TRT）；CosyVoice-300M-25Hz 权重已下载（zero-shot，无内置音色，tts.py 已统一为零样本/SFT 双路径），**RTF 对比还没跑完**。tts.py 会 torchaudio.load 的 tensor 直通补丁（CosyVoice 官方代码对已加载 tensor 重复调用 load_wav）。
